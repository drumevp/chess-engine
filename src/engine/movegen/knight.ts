...
import { KNIGHT_INDEX, NUMBER_OF_PIECE_CATEGORIES } from "../constants/piece";
...
const generateKnightMoves = (
  ...
  const stateIndex = ctx.color * NUMBER_OF_PIECE_CATEGORIES + KNIGHT_INDEX;
  ...
  const colorAndPieceBits = (ctx.color << 12) | (KNIGHT_INDEX << 13);
  ...
  while (true) {
    while (originBits !== 0) {
      ...
      const pseudoTargetsLo =
        (knightAttacksLo[originSquare] & ~ctx.ownOccupancyLo) >>> 0;
      const pseudoTargetsHi =
        (knightAttacksHi[originSquare] & ~ctx.ownOccupancyHi) >>> 0;
