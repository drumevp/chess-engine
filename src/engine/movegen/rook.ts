...
import { NUMBER_OF_PIECE_CATEGORIES, ROOK_INDEX } from "../constants/piece";
...
const generateRookMoves = (
  ...
  const stateIndex = ctx.color * NUMBER_OF_PIECE_CATEGORIES + ROOK_INDEX;
  ...
  const colorAndPieceBits = (ctx.color << 12) | (ROOK_INDEX << 13);
  ...
  while (true) {
    while (originBits !== 0) {
      ...
      const tableIndex = rookMagicAttackOffsets[originSquare] + magicIndex;
      const pseudoTargetsLo =
        (rookMagicAttacksLo[tableIndex] & ~ctx.ownOccupancyLo) >>> 0;
      const pseudoTargetsHi =
        (rookMagicAttacksHi[tableIndex] & ~ctx.ownOccupancyHi) >>> 0;
