...
import generateAttackInfo, { createAttackInfo } from "../engine/movegen/attackInfo/main";
import generateLegalMovesFromContext from "../engine/movegen/generateLegalMovesFromContext";
import getMoveGenerationContext, { createMoveGenerationContext } from "../engine/movegen/getMoveGenerationContext";
...
const failSoftAlphaBetaNegaMax = (position: Position, alpha: number, beta: number, depth: number): number => {
  const searchPlyCount = depth + 1;
  const moveLists = Array.from({ length: searchPlyCount }, () => createMoveList());
  const contexts = moveLists.map((moveList) =>
    createMoveGenerationContext(moveList),
  );
  const attackInfos = Array.from({ length: searchPlyCount }, () => createAttackInfo());
  const undoStack = Array.from({ length: searchPlyCount }, () => createUndo());
  let repetitionCounts: Map<bigint, number> = new Map();
  repetitionCounts.set(position.zobristHash, 1);
  const gameStateScratch: DetermineGameStateRValue = { gameState: GAME_STATE.ONGOING, gameEndReason: null };
  const originalColor = position.color;

  return failSoftAlphaBetaNegaMaxRecursive(position, alpha, beta, depth, 0, moveLists, contexts, attackInfos, undoStack, gameStateScratch, repetitionCounts, originalColor);
}

const failSoftAlphaBetaNegaMaxRecursive = (
  position: Position, 
  alpha: number, 
  beta: number, depth: number, 
  ply: number, 
  moveLists: MoveList[],
  ...
  originalColor: ColorType): number => {
  ...
  determineGameState(position, repetitionCounts, movesCount, isCheck, gameStateScratch);
  ...
  for (let i = 0; i < movesCount; i++) {
    ...
    const score = -failSoftAlphaBetaNegaMaxRecursive(position, -beta, -alpha, depth - 1, ply + 1, moveLists, contexts, attackInfos, undoStack, gameStateScratch, repetitionCounts, originalColor);
